...
describe("away", function()
    ...
    describe("spawn()", function()
        ...
        it("use current sched if first arg is nil", function ()
            local sched = away.sched()
            away.spawn(sched, function ()
                local th = away.spawn(nil, function ()
                    away.stop(sched)
                end)
                assert.is_thread(th)
            end)
        ...
        it("will copy context from sched if first arg present", function ()
            local sched = away.sched()
            local rootcx = away.root_context(sched)
            rootcx.test = 0
            away.spawn(sched, function ()
                local cx = away.context()
                cx.test = 1
                away.spawn(sched, function ()
                    local cx = away.context()
                    assert.are.equal(0, cx.test)
                    away.stop(sched)
                end)
            end)
            away.run(sched)
        ...
        it("will copy context from current thread if first arg is nil", function ()
            local sched = away.sched()
            local rootcx = away.root_context(sched)
            rootcx.test = 0
            away.spawn(sched, function ()
                local cx = away.context()
                cx.test = 1
                away.spawn(nil, function ()
                    local cx = away.context()
                    assert.are.equal(1, cx.test)
                    away.stop(sched)
                end)
            end)
            away.run(sched)
    ...
    describe("context()", function ()
        it("can get value from root context", function ()
            local sched = away.sched()
            local rootcx = away.root_context(sched)
            rootcx.testvalue = "Hello World"
            away.spawn(sched, function ()
                local cx = away.context()
                assert.are.equal("Hello World", cx.testvalue)
                away.stop(sched)
            end)
            away.run(sched)
        end)

        it("could not overwrite root context from inner context", function ()
            local sched = away.sched()
            local rootcx = away.root_context(sched)
            rootcx.testvalue = "Hello World"
            away.spawn(sched, function ()
                local cx = away.context()
                cx.testvalue = "Goodbye World"
                assert.are.equal("Goodbye World", cx.testvalue)
                away.stop(sched)
            end)
            away.run(sched)
            assert.are.equal("Hello World", rootcx.testvalue)
        end)
    end)
end)
